...
const InventoryPage: React.FC = () => {
  ...
  // Statistics for this filtered view
  ...
  const getFilterIcon = (filterType: FilterType) => {
    switch (filterType) {
      case 'A1-A6':
        return <MapPin className="w-5 h-5 text-fedshi-purple" />;
      case 'A7-A12':
        return <Warehouse className="w-5 h-5 text-fedshi-purple" />;
      case 'B-AG':
        return <Package className="w-5 h-5 text-fedshi-purple" />;
      default:
        return null;
    }
  };
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-fedshi-purple/10 font-inter">
        <div className="bg-white/90 backdrop-blur-sm shadow-lg border-b border-gray-200">
          <div className="px-4 sm:px-6 py-6 sm:py-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2 sm:space-x-4">
                ...
                <div className="flex items-center space-x-2 sm:space-x-3">
                  ...
                  <h1 className="text-lg sm:text-3xl font-bold text-gray-900 font-inter">{title}</h1>
                </div>
              </div>
            ...
            <div className="mt-4 sm:mt-6 flex flex-wrap items-center gap-2 sm:gap-6 text-xs sm:text-sm text-gray-600">
              ...
              <span>Total Items: <strong className="text-gray-900 text-sm sm:text-lg">{searchedData.length}</strong>{searchTerm && <span className="text-gray-500"> (من {filteredData.length})</span>}</span>
              <span className="hidden sm:inline">•</span>
              <span>Filter: <strong className="text-fedshi-purple text-sm sm:text-lg">{filterType}</strong></span>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 sm:px-6 py-6 sm:py-8">
          ...
          {/* Notes Summary Table - Only for this filtered view */}
